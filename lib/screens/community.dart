...
class _CommunityPageState extends State<CommunityPage> {
  ...
  late bool? _eulaAccepted = null; // Flag to track EULA acceptance
  ...
  Future<void> _loadPosts() async {
    ...
    if (postsData != null) {
      ...
      final filteredPostsList = postsList.where((post) {
        final userId = post['userId'];
        return userId != null && !_blockedUsers.contains(userId as String);
      }).toList();

      // Sort the filtered posts list
      filteredPostsList.sort((a, b) {
        final int timestampA = int.tryParse(a['timestamp'] as String? ?? '0') ?? 0;
        final int timestampB = int.tryParse(b['timestamp'] as String? ?? '0') ?? 0;
  ...
  Future<void> _submitPost() async {
    ...
    if (_postController.text.trim() != "" &&
      ...
      if (user != null) {
        ...
        print('This users url: $profilePicURL');
  ...
  Widget _buildPostTile(Map<String, dynamic> post) {
    //Get posters ID and users Id to test if they are the same
    final userPostID = post['userId'] ?? '';
    //Get other post data
    final username = post['username'] ?? 'anonymous';
    final profilePicURL = post['profilePicURL'] ?? 'assets/images/profile_pictures/avatar1.png';
      ...
      children: [
        ...
        Row(
          children: [
            ...
            if(userPostID != user?.uid)
              IconButton(
                onPressed: () {
                  print('Posters ID: ${userPostID} = Users ID: ${user?.uid}: ${userPostID != user?.uid}');
                  setState(() {
                    isFlagged = !isFlagged;
                    if (isFlagged) {
                      showDialog(
            ...
            //If user ID = this post ID Don't show Icon
            if(userPostID != user?.uid)
              IconButton(
                onPressed: () {
                  showDialog(
